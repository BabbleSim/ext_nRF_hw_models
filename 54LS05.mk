...
HW_SRCS_FILE=make_inc/54LS05_hw_files
HAL_FLPR_SRCS_FILE=make_inc/54LS05_flpr_hal_files
HAL_APP_SRCS_FILE=make_inc/54LS05_app_hal_files

SRCS_HW=$(shell cat ${HW_SRCS_FILE})
SRCS_HAL_FLPR=$(shell cat ${HAL_FLPR_SRCS_FILE})
SRCS_HAL_APP=$(shell cat ${HAL_APP_SRCS_FILE})

ifneq (,$(findstring hal_app,$(MAKECMDGOALS)))
 SRCS:=${SRCS_HAL_APP}
 LIB_VARIANT:=_hal_app
 HAL_VARIANT=-DNRF54LS05A_XXAA -DNRF54LS05A_ENGA_XXAA -DNRF_APPLICATION
else #hw
 SRCS:=${SRCS_HW}
 LIB_VARIANT:=
 HAL_VARIANT=-DNRF54LS05A_XXAA -DNRF54LS05A_ENGA_XXAA -DNRF_APPLICATION
endif
VARIANT=NRF54LS05${LIB_VARIANT}
...
A_LIBS32:=
SO_LIBS:=
CFLAGS=${COMMON_CC_FLAGS} ${INCLUDES} -DNRF54LS05 ${HAL_VARIANT}
