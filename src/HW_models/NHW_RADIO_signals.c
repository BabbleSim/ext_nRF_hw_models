...
#define _RADIO_CHECK_INTERRUPT(event, inten, intenset) \
...
#if NHW_RADIO_IS_54
  #define RADIO_CHECK_INTERRUPT(event, inten, regnb) \
    _RADIO_CHECK_INTERRUPT(event, inten, INTENSET##regnb##_ )
#else
  #define RADIO_CHECK_INTERRUPT(event, inten, regnb) \
  ...
  for (int i = 0; i < NHW_RADIO_N_INT; i ++) {
    ...
    RADIO_CHECK_INTERRUPT(READY, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(ADDRESS, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(PAYLOAD, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(END, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(DISABLED, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(DEVMATCH, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(DEVMISS, RADIO_INTEN[i], 00)
#if !NHW_RADIO_IS_54
    RADIO_CHECK_INTERRUPT(RSSIEND, RADIO_INTEN[i], 00)
#endif
    RADIO_CHECK_INTERRUPT(BCMATCH, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(CRCOK, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(CRCERROR, RADIO_INTEN[i], 00)
#if NHW_RADIO_HAS_15_4
    RADIO_CHECK_INTERRUPT(FRAMESTART, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(EDEND, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(EDSTOPPED, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(CCAIDLE, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(CCABUSY, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(CCASTOPPED, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(MHRMATCH, RADIO_INTEN[i], 00)
#endif
#if NHW_RADIO_HAS_BLECODED
    RADIO_CHECK_INTERRUPT(RATEBOOST, RADIO_INTEN[i], 00)
#endif
    RADIO_CHECK_INTERRUPT(TXREADY, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(RXREADY, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(SYNC, RADIO_INTEN[i], 00)
    RADIO_CHECK_INTERRUPT(PHYEND, RADIO_INTEN[i], 00)
#if NHW_RADIO_HAS_DF
    RADIO_CHECK_INTERRUPT(CTEPRESENT, RADIO_INTEN[i], 00)
#endif
#if defined(RADIO_INTENSET01_PLLREADY_Msk)
    RADIO_CHECK_INTERRUPT(PLLREADY, RADIO_INTEN[i], 01)
#endif
...
static NHW_SIGNAL_EVENT_ns_si(RADIO, PLLREADY)
#endif
...
#if defined(RADIO_SHORTS_PHYEND_PLLEN_Msk)
  NHW_SHORT_si(RADIO, PHYEND, PLLEN)
#endif
}
...
#endif
#if NHW_RADIO_HAS_PLLEN
NHW_SIDEEFFECTS_TASKS_si(RADIO, PLLEN)
#endif
...
#endif
#if NHW_RADIO_HAS_PLLEN
NHW_SIDEEFFECTS_SUBSCRIBE_si(RADIO, PLLEN)
#endif
#endif /* NHW_HAS_DPPI */
